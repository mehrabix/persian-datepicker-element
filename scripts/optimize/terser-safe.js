...
// Files to optimize
const files = ['dist/persian-datepicker-element.min.js', 'dist/persian-datepicker-element.esm.js'];
...
async function optimizeFile(filePath, isModule = false) {
  try {
    ...
    console.log(
      `First pass reduction: ${reduction}% (${(originalSize / 1024).toFixed(2)} KB -> ${(intermediateSize / 1024).toFixed(2)} KB)`
    );
    ...
    if (finalSize >= originalSize) {
      console.log(
        `⚠️ Final minification did not reduce size (${(originalSize / 1024).toFixed(2)} KB -> ${(finalSize / 1024).toFixed(2)} KB)`
      );
      console.log('Restoring original file...');
      fs.copyFileSync(backupPath, filePath);
    } else {
      ...
      fs.writeFileSync(filePath, result.code);
      console.log(
        `✅ Size reduced by ${reduction}% (${(originalSize / 1024).toFixed(2)} KB -> ${(finalSize / 1024).toFixed(2)} KB)`
      );
      ...
      try {
        ...
        if (!advancedResult.error) {
          ...
          if (advancedSize < finalSize) {
            ...
            console.log(
              `🔥 Advanced settings reduced size by an additional ${advancedReduction}% (${(finalSize / 1024).toFixed(2)} KB -> ${(advancedSize / 1024).toFixed(2)} KB)`
            );
          } else {
