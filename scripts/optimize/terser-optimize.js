...
async function optimizeFile(filePath, isModule = false) {
  try {
    ...
    if (minifiedSize >= originalSize) {
      console.log(
        `⚠️ Warning: Minification did not reduce file size (${(originalSize / 1024).toFixed(2)} KB -> ${(minifiedSize / 1024).toFixed(2)} KB)`
      );
      console.log('Restoring original file...');
      fs.copyFileSync(backupPath, filePath);
    } else {
      ...
      console.log(
        `✅ Size reduced by ${reduction}% (${(originalSize / 1024).toFixed(2)} KB -> ${(minifiedSize / 1024).toFixed(2)} KB)`
      );
    }
