...
function safeMinify(filePath, terserCmd) {
  ...
  try {
    ...
    if (sizeAfter > sizeBefore) {
      console.log(
        `⚠️ Warning: File size increased after minification (${(sizeBefore / 1024).toFixed(2)}KB -> ${(sizeAfter / 1024).toFixed(2)}KB)`
      );
    ...
    try {
      if (output.file === 'persian-datepicker-element.min.js') {
        ...
        try {
          ...
          execSync(
            `powershell -Command "Get-Content '${tempJsPath}' | Compress-Archive -DestinationPath '${outputPath}.zip' -Force"`,
            { stdio: 'inherit' }
          );
