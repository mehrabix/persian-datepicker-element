...
describe('EventUtils', () => {
  ...
  beforeAll(() => {
    ...
    EventUtils.getEvents = jest.fn(
      (month: number, day: number, eventTypes?: string[], includeAllTypes: boolean = false) => {
  ...
  describe('Event Loading', () => {
    ...
    test('should load religious events', () => {
      ...
      if (religiousEvents.length > 0) {
        ...
        console.warn(
          'No religious events found in the current dataset - this is not necessarily an error'
        );
      }
    });
  });
  ...
  describe('Hijri Event Mapping', () => {
    test('should refresh events', () => {
      ...
      (HijriUtils.getHijriEventDateInJalaliYear as jest.Mock).mockImplementation(
        (year, month, day) => {
  ...
  describe('getHolidayTitles', () => {
    beforeEach(() => {
      EventUtils.getHolidayTitles = jest.fn(
        (month: number, day: number, eventTypes?: string[], includeAllTypes: boolean = false) => {
          const events = EventUtils.getEvents(month, day, eventTypes, includeAllTypes);
  ...
  describe('getAllEventTitles', () => {
    beforeEach(() => {
      EventUtils.getAllEventTitles = jest.fn(
        (month: number, day: number, eventTypes?: string[], includeAllTypes: boolean = false) => {
          const events = EventUtils.getEvents(month, day, eventTypes, includeAllTypes);
