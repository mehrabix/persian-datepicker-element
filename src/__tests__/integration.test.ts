...
describe('Persian Date Picker Element Integration', () => {
  ...
  test.skip('component correctly handles date selection and emits events', async () => {
    ...
    // Select a day
    const dayElements = pickerElement.shadowRoot?.querySelectorAll(
      '.day:not(.empty)'
  ...
  test('component properly navigates between months', async () => {
    ...
    const nextButton = pickerElement.shadowRoot?.querySelector(
      '.next-btn, .next, [aria-label="Next month"]'
    ) as HTMLElement;
    if (!nextButton) {
      throw new Error(
        'Next month button not found. Test needs to be updated to match component structure.'
      );
    }
    ...
    const prevButton = pickerElement.shadowRoot?.querySelector(
      '.prev-btn, .prev, [aria-label="Previous month"]'
    ) as HTMLElement;
    if (!prevButton) {
      throw new Error(
        'Previous month button not found. Test needs to be updated to match component structure.'
      );
    }
