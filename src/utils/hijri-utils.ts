...
export function hijriToGregorian(hy: number, hm: number, hd: number): [number, number, number] {
  ...
  const epochDays =
    Math.floor((hy - 1) * 354.367) + // Average Hijri year is 354.367 days
 ...
 */
export function getHijriEventDateInJalaliYear(
  ...
  const currentHijriYear =
    Math.floor(currentPersianYear - 621.5 + 0.74 * (currentPersianYear - 621.5)) - 202;
  ...
  for (let yearOffset = 0; yearOffset <= 1; yearOffset++) {
    try {
      ...
      console.warn(
        `Error converting Hijri date (${currentHijriYear}, ${hijriMonth}, ${hijriDay})`,
        error
      );
    }
  }
