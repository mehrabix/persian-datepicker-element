...
try {
  ...
  Object.keys(allHolidaysByMonth)
    .sort((a, b) => a - b)
    .forEach(month => {
  ...
  Object.keys(iranHolidaysByMonth)
    .sort((a, b) => a - b)
    .forEach(month => {
  ...
  Object.keys(religiousHolidaysByMonth)
    .sort((a, b) => a - b)
    .forEach(month => {
  ...
  Object.keys(afghanistanHolidaysByMonth)
    .sort((a, b) => a - b)
    .forEach(month => {
  ...
  const persianAfghanHolidays = eventsData['Persian Calendar'].filter(
    event => event.type === 'Afghanistan' && event.holiday === true
  );
  ...
  const hijriAfghanHolidays = eventsData['Hijri Calendar'].filter(
    event => event.type === 'Afghanistan' && event.holiday === true
  );
  ...
  function isHoliday(month, day, types) {
    return allHolidays.some(
      holiday =>
        holiday.month === month && holiday.day === day && (!types || types.includes(holiday.type))
    );
  }
  ...
  console.log(
    `Is holiday (Iran type): ${isHoliday(iranHolidayDate.month, iranHolidayDate.day, ['Iran'])}`
  );
  console.log(
    `Holiday titles (Iran type): ${getHolidayTitles(iranHolidayDate.month, iranHolidayDate.day, ['Iran'])}`
  );
  ...
  console.log(
    `Is holiday (Afghanistan type): ${isHoliday(afghanHolidayDate.month, afghanHolidayDate.day, ['Afghanistan'])}`
  );
  console.log(
    `Holiday titles (Afghanistan type): ${getHolidayTitles(afghanHolidayDate.month, afghanHolidayDate.day, ['Afghanistan'])}`
  );
  ...
  console.log(
    `Is holiday (Afghanistan type): ${isHoliday(afghanHijriHolidayDate.month, afghanHijriHolidayDate.day, ['Afghanistan'])}`
  );
  console.log(
    `Holiday titles (Afghanistan type): ${getHolidayTitles(afghanHijriHolidayDate.month, afghanHijriHolidayDate.day, ['Afghanistan'])}`
  );
  ...
  console.log(
    `Afghanistan holidays: ${afghanistanHolidays.length} (Persian: ${persianAfghanHolidays.length}, Hijri: ${hijriAfghanHolidays.length})`
  );
} catch (error) {
